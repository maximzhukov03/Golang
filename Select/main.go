...
func main(){
    ...
    users := make(chan User) //Создание канала

    go generateUsers(50, users) // Генерация юзеров(канал) при помощи функции

    wg := &sync.WaitGroup{} // Созание группы для горутин

    for user := range users{ //Итерируемся по юзерам чтобы записать их в файл
        wg.Add(1)
...
func generateUsers(count int, users chan User){//Функция для генерации юзеров (На вход подается канал)
    for i := 0; i < count; i++{
        users <- User{ // Здесь вместо слайса используется канал
            id: i + 1,
    ...
    close(users) //ОБЯЗАТЕЛЬНО ЗАКРЫТЬ ЕГО
}
